...
# ป้องกัน decompression bomb (PIL เปิดภาพใหญ่มากจนกิน RAM หมด)
# ตั้งเกินไว้เยอะ ๆ เพื่อให้ check ของเราเป็นคนตอบ error ที่เป็นมิตร
# (จริง ๆ bomb attack มักเป็นระดับ gigapixel)
Image.MAX_IMAGE_PIXELS = 100_000_000  # 100 MP hard cap
...
/* Lightbox fullscreen — CSS :target + JS ESC handler */
.lightbox {
    position: fixed;
    inset: 0;
...
.lightbox:target img { transform: scale(1); }
.lightbox .lb-close {
    position: absolute;
    inset: 0;
    display: block;
...
.lightbox .lb-x:hover { background: rgba(255, 255, 255, 0.2); }
.lightbox .lb-hint {
    position: absolute;
    bottom: 24px;
    left: 50%;
...
.zoom-thumb:hover { transform: scale(1.015); }
...
def _process_one(img, cut_mode, target_aspect):
    ...
    # Trim รายรูป (ไม่บังคับเท่ากัน) → ไม่กินเนื้อหาของรูปที่ไม่มีขอบ
    for c, (t, b, l, r) in zip(cells, [_detect_trim(c['image'], border_val) for c in cells]):
        if t or b or l or r:
            w, h = c['image'].size
            c['image'] = c['image'].crop((l, t, w - r, h - b))

    # ย่อทุกรูปลงเท่าขนาดเล็กสุด (center-crop) → ทุกรูปเท่ากัน
    if cells:
        min_w = min(c['image'].size[0] for c in cells)
        min_h = min(c['image'].size[1] for c in cells)
        for c in cells:
            w, h = c['image'].size
            if w > min_w or h > min_h:
                dx = (w - min_w) // 2
                dy = (h - min_h) // 2
                c['image'] = c['image'].crop((dx, dy, dx + min_w, dy + min_h))
...
def _process_manual(img, rows, cols, h_pcts, v_pcts, cut_mode, target_aspect):
    """Manual override: ใช้ตำแหน่งเส้นแบ่งที่ user กำหนด (percent 0-100)"""
    W, H = img.size
    h_pts = sorted({max(1, min(H - 1, int(p / 100.0 * H))) for p in h_pcts})
    v_pts = sorted({max(1, min(W - 1, int(p / 100.0 * W))) for p in v_pcts})
    h_bounds = [0] + h_pts + [H]
    v_bounds = [0] + v_pts + [W]
    h_ranges = [(h_bounds[i], h_bounds[i + 1]) for i in range(len(h_bounds) - 1)]
    v_ranges = [(v_bounds[i], v_bounds[i + 1]) for i in range(len(v_bounds) - 1)]

    cells, cw, ch = cut_image_by_ranges(img, h_ranges, v_ranges, cut_mode, target_aspect)
    ...
    if cells:
        ...
        for c in cells:
            iw, ih = c['image'].size
            if (iw, ih) != (min_w, min_h):
                x0 = (iw - min_w) // 2
                y0 = (ih - min_h) // 2
                c['image'] = c['image'].crop((x0, y0, x0 + min_w, y0 + min_h))
        cw, ch = min_w, min_h

    dbg = {'method': 'manual', 'rows': rows, 'cols': cols,
           'h_pcts': list(h_pcts), 'v_pcts': list(v_pcts),
           'border_val': None, 'megapixels': round(W * H / 1e6, 2)}
    return cells, cw, ch, None, dbg


def _manual_preview(img, h_pcts, v_pcts, max_w=900):
    """วาดเส้นแดงทับรูปเพื่อพรีวิวตำแหน่งตัด"""
    W, H = img.size
    ...
    for p in h_pcts:
        y = int(p / 100.0 * ph)
        draw.line([(0, y), (pw, y)], fill=(255, 40, 40), width=lw)
    for p in v_pcts:
        x = int(p / 100.0 * pw)
        draw.line([(x, 0), (x, ph)], fill=(255, 40, 40), width=lw)
    return prev
...
def _render_result_card(uf, img, cells, cw, ch, border_val, cut_mode, idx, debug=None, naming_mode="sequential"):
    ...
    with st.container(border=True):
        ...
        # Preview — grid ตามโครงสร้างจริง + ปุ่มดูใหญ่/ดาวน์โหลดต่อภาพ
        cells_sorted = sorted(cells, key=lambda c: (c['row'], c['col']))
        pad_n = max(2, len(str(len(cells_sorted))))

        def _cell_filename(cell, n):
            if naming_mode == "sequential":
                return f"{base_name}_{str(n).zfill(pad_n)}.png"
            if cut_mode == "vertical":
                return f"{base_name}_col{cell['col']:02d}.png"
            if cut_mode == "horizontal":
                return f"{base_name}_row{cell['row']:02d}.png"
            return f"{base_name}_r{cell['row']:02d}c{cell['col']:02d}.png"
        ...
        for i in range(0, len(cells_sorted), preview_cols):
            ...
            for j, cell in enumerate(cells_sorted[i:i+preview_cols]):
                ...
                with row[j]:
                    ...
                    lb_id = f"lb_{idx}_{n}"
                    # คลิกภาพ → overlay fullscreen (CSS :target, ไม่ re-run)
                    st.markdown(
                        f'<a href="#{lb_id}" title="คลิกเพื่อดูขนาดเต็ม">'
                        f'<img class="zoom-thumb" src="data:image/png;base64,{b64}"/>'
                        f'</a>'
                        f'<div id="{lb_id}" class="lightbox">'
                        f'<a href="#_" class="lb-close"></a>'
                        f'<a href="#_" class="lb-x" title="ปิด (ESC)">✕</a>'
                        f'<img src="data:image/png;base64,{b64}"/>'
                        f'<div class="lb-hint">ESC หรือคลิกพื้นหลังเพื่อปิด</div>'
                        f'</div>',
                        unsafe_allow_html=True,
                    )
                    st.download_button(
                        f"⬇ {fn}", data=png_bytes, file_name=fn,
                        mime="image/png", key=f"dl1_{idx}_{n}",
                        use_container_width=True,
                    )

        # Download รวม
        ordered = cells_sorted
        pad = pad_n
        zip_buf = io.BytesIO()
        with zipfile.ZipFile(zip_buf, 'w', zipfile.ZIP_DEFLATED) as zf:
            for n, cell in enumerate(ordered, start=1):
                ...
                zf.writestr(_cell_filename(cell, n), b.getvalue())
        zip_buf.seek(0)
        st.download_button(
...
def main():
    ...
    with st.sidebar:
        ...
        mode_label = st.radio(
            ...
            index=0,
            horizontal=True,
            help="Auto = ตรวจจับอัตโนมัติ · Manual = กำหนดเส้นแบ่งเอง",
        )
        manual_mode = mode_label == "Manual"
        ...
        if manual_mode:
            st.markdown("---")
            st.markdown("### Manual grid")
            c1, c2 = st.columns(2)
            with c1:
                manual_rows = st.number_input("Rows", 1, 10, 3, key="manual_rows")
            with c2:
                manual_cols = st.number_input("Cols", 1, 10, 3, key="manual_cols")

            manual_uniform = st.checkbox(
                "แบ่งเท่ากัน", value=True,
                help="เอาออกเพื่อปรับตำแหน่งเส้นแบ่งเอง",
            )

            if manual_uniform:
                manual_h_pcts = [round(100 * i / manual_rows, 2) for i in range(1, manual_rows)]
                manual_v_pcts = [round(100 * i / manual_cols, 2) for i in range(1, manual_cols)]
            else:
                st.caption("ตำแหน่งเส้นแบ่ง (% ของภาพ)")
                manual_h_pcts = []
                for i in range(1, manual_rows):
                    default = round(100 * i / manual_rows, 1)
                    manual_h_pcts.append(
                        st.slider(f"เส้นนอน #{i}", 1.0, 99.0, default, 0.1, key=f"h_{i}")
                    )
                manual_v_pcts = []
                for j in range(1, manual_cols):
                    default = round(100 * j / manual_cols, 1)
                    manual_v_pcts.append(
                        st.slider(f"เส้นตั้ง #{j}", 1.0, 99.0, default, 0.1, key=f"v_{j}")
                    )

        st.markdown("---")
        st.caption(
            "Auto: ตรวจจับขอบอัตโนมัติ · Manual: กำหนดเส้นเอง · "
            "ทุกโหมด crop ให้ได้ aspect ratio ที่เลือก"
        )
    ...
    # ESC key → ปิด lightbox (inject ลง parent document)
    components.html(
        """
        <script>
        (function() {
            const doc = window.parent.document;
            if (doc._lbEscBound) return;
            doc._lbEscBound = true;
            doc.addEventListener('keydown', function(e) {
                if (e.key === 'Escape' && window.parent.location.hash &&
                    window.parent.location.hash.startsWith('#lb_')) {
                    history.pushState('', doc.title, window.parent.location.pathname + window.parent.location.search);
                }
            });
        })();
        </script>
    ...
    st.caption(
        f"ขีดจำกัด: ≤ {MAX_FILE_MB} MB/ไฟล์ · ≤ 6K (6144×3456) · "
        f"สูงสุด {MAX_FILES_PER_BATCH} ไฟล์/ครั้ง"
    )
    ...
    if len(uploaded) > MAX_FILES_PER_BATCH:
        st.error(f"อัพได้ครั้งละไม่เกิน {MAX_FILES_PER_BATCH} ไฟล์ (ส่งมา {len(uploaded)})")
        return

    for idx, uf in enumerate(uploaded):
        size_mb = getattr(uf, "size", 0) / (1024 * 1024)
        if size_mb > MAX_FILE_MB:
            failed.append((uf.name, f"ไฟล์ใหญ่เกิน {MAX_FILE_MB} MB ({size_mb:.1f} MB)"))
            continue

        ext = Path(uf.name).suffix.lower().lstrip(".")
        if ext not in EXT_TO_FORMAT:
            failed.append((uf.name, f"นามสกุลไฟล์ไม่รองรับ ({ext})"))
            continue

        raw = uf.getvalue() if hasattr(uf, "getvalue") else uf.read()
        if len(raw) > MAX_FILE_MB * 1024 * 1024:
            failed.append((uf.name, f"ไฟล์ใหญ่เกิน {MAX_FILE_MB} MB"))
            continue

        try:
            verify_img = Image.open(io.BytesIO(raw))
            verify_img.verify()  # ตรวจว่าเป็นภาพจริง ไม่ใช่ไฟล์ปลอม
        except Exception as e:
            failed.append((uf.name, f"ไฟล์เสียหรือไม่ใช่ภาพจริง: {e}"))
            continue
        ...
        try:
            ...
            failed.append((uf.name, "ภาพใหญ่เกินกว่าขีดจำกัด (decompression bomb)"))
            continue
        except Exception as e:
        ...
        if pixels > MAX_PIXELS_4K:
            failed.append((
                uf.name,
                f"ความละเอียดเกิน 6K ({img.size[0]}×{img.size[1]} ≈ {pixels/1e6:.1f} MP, สูงสุด 21.2 MP)"
            ))
            continue
        ...
        if img.mode not in ("RGB", "RGBA", "L"):
            try:
                ...
                failed.append((uf.name, f"แปลง color mode ไม่ได้: {e}"))
                continue

        with st.spinner(f"กำลังประมวลผล {uf.name}..."):
            if manual_mode:
                cells, cw, ch, border_val, dbg = _process_manual(
                    img, manual_rows, manual_cols,
                    manual_h_pcts, manual_v_pcts,
                    cut_mode, target_aspect,
                )
            else:
                cells, cw, ch, border_val, dbg = _process_one(img, cut_mode, target_aspect)

        if manual_mode:
            with st.expander(f"🎯 พรีวิวเส้นแบ่ง — {uf.name}", expanded=(idx == 0)):
                st.image(
                    _manual_preview(img, manual_h_pcts, manual_v_pcts),
                    caption=f"เส้นแดง = ตำแหน่งตัด · {manual_rows}×{manual_cols} = {manual_rows * manual_cols} ช่อง",
                    use_container_width=True,
                )
