...
MAX_FILE_MB = 10
MAX_PIXELS_4K = 3840 * 2160  # ~8.3 MP
...
Image.MAX_IMAGE_PIXELS = MAX_PIXELS_4K + 100_000
        ...
        for i in range(0, len(cells_sorted), preview_cols):
            ...
            for j, cell in enumerate(cells_sorted[i:i+preview_cols]):
                ...
                with row[j]:
                    ...
                    with btn_cols[0]:
                        if st.button("🔍 ดูใหญ่", key=f"zoom_{idx}_{n}", use_container_width=True):
                            st.session_state[f"preview_{idx}_{n}"] = True
                    with btn_cols[1]:
                        st.download_button(
                    ...
                    if st.session_state.get(f"preview_{idx}_{n}"):
                        @st.dialog(fn, width="large")
                        def _show():
                            st.image(cell['image'], use_container_width=True)
                            st.caption(f"ขนาด: {cell['image'].size[0]}×{cell['image'].size[1]}px")
                        _show()
                        st.session_state[f"preview_{idx}_{n}"] = False
...
def main():
    ...
    st.caption(
        f"ขีดจำกัด: ≤ {MAX_FILE_MB} MB/ไฟล์ · ≤ 4K (3840×2160) · "
    ...
    for idx, uf in enumerate(uploaded):
        ...
        if pixels > MAX_PIXELS_4K:
            failed.append((
                uf.name,
                f"ความละเอียดเกิน 4K ({img.size[0]}×{img.size[1]} ≈ {pixels/1e6:.1f} MP, สูงสุด 8.3 MP)"
            ))
            continue
